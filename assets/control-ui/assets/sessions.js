import{f as e,r as t,u as n}from"./i18n.js";import{l as r}from"./format.js";import{A as i,N as a,s as o}from"./index.js";var s=[``,`off`,`minimal`,`low`,`medium`,`high`,`xhigh`],c=[``,`off`,`on`],l=[{value:``,label:`inherit`},{value:`off`,label:`off (explicit)`},{value:`on`,label:`on`},{value:`full`,label:`full`}],u=[{value:``,label:`inherit`},{value:`on`,label:`on`},{value:`off`,label:`off`}],d=[``,`off`,`on`,`stream`],f=[10,25,50,100];function p(e){if(!e)return``;let t=e.trim().toLowerCase();return t===`z.ai`||t===`z-ai`?`zai`:t}function m(e){return p(e)===`zai`}function h(e){return m(e)?c:s}function g(e,t){return!t||e.includes(t)?[...e]:[...e,t]}function _(e,t){return!t||e.some(e=>e.value===t)?[...e]:[...e,{value:t,label:`${t} (custom)`}]}function v(e,t){return!t||!e||e===`off`?e:`on`}function y(e,t){return e?t&&e===`on`?`low`:e:null}function b(e,t){let n=t.trim().toLowerCase();return n?e.filter(e=>{let t=(e.key??``).toLowerCase(),r=(e.label??``).toLowerCase(),i=(e.kind??``).toLowerCase(),a=(e.displayName??``).toLowerCase();return t.includes(n)||r.includes(n)||i.includes(n)||a.includes(n)}):e}function x(e,t,n){let r=n===`asc`?1:-1;return[...e].toSorted((e,n)=>{let i=0;switch(t){case`key`:i=(e.key??``).localeCompare(n.key??``);break;case`kind`:i=(e.kind??``).localeCompare(n.kind??``);break;case`updated`:i=(e.updatedAt??0)-(n.updatedAt??0);break;case`tokens`:i=(e.totalTokens??e.inputTokens??e.outputTokens??0)-(n.totalTokens??n.inputTokens??n.outputTokens??0);break}return i*r})}function S(e,t,n){let r=t*n;return e.slice(r,r+n)}function C(r){let a=x(b(r.result?.sessions??[],r.searchQuery),r.sortColumn,r.sortDir),o=a.length,s=Math.max(1,Math.ceil(o/r.pageSize)),c=Math.min(r.page,s-1),l=S(a,c,r.pageSize),u=(t,n,a=``)=>{let o=r.sortColumn===t,s=o&&r.sortDir===`asc`?`desc`:`asc`;return e`
  ...
  `}function w(i,s,c,f,p,b,x){let S=i.updatedAt?r(i.updatedAt):t(`common.na`),C=i.thinkingLevel??``,w=m(i.modelProvider),T=v(C,w),E=g(h(i.modelProvider),T),D=i.fastMode===!0?`on`:i.fastMode===!1?`off`:``,O=_(u,D),k=i.verboseLevel??``,A=_(l,k),j=i.reasoningLevel??``,M=g(d,j),N=typeof i.displayName==`string`&&i.displayName.trim().length>0?i.displayName.trim():null,P=!!(N&&N!==i.key&&N!==(typeof i.label==`string`?i.label.trim():``)),F=i.kind!==`global`,I=F?`${a(`chat`,s)}?session=${encodeURIComponent(i.key)}`:null,L=i.kind===`direct`?`data-table-badge--direct`:i.kind===`group`?`data-table-badge--group`:i.kind===`global`?`data-table-badge--global`:`data-table-badge--unknown`;return e`
    <tr>
      ...
      <td class="data-table-key-col">
        <div class="mono session-key-cell">
          ${F?e`<a
                href=${I}
                class="session-link"
                @click=${e=>{e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey||e.altKey||x&&(e.preventDefault(),x(i.key))}}
                >${i.key}</a
              >`:i.key}
      ...
      <td>
        <input
          ...
          @change=${e=>{let t=e.target.value.trim();c(i.key,{label:t||null})}}
        />
      </td>
      <td>
        <span class="data-table-badge ${L}">${i.kind}</span>
      </td>
      <td>${S}</td>
      <td>${o(i)}</td>
      <td>
        <select
          ...
          @change=${e=>{let t=e.target.value;c(i.key,{thinkingLevel:y(t,w)})}}
        >
      ...
      <td>
        <select
          ?disabled=${b}
          style="padding: 6px 10px; font-size: 13px; border: 1px solid var(--border); border-radius: var(--radius-sm); min-width: 90px;"
          @change=${e=>{let t=e.target.value;c(i.key,{fastMode:t===``?null:t===`on`})}}
        >
      ...
      <td>
        <select
          ?disabled=${b}
          style="padding: 6px 10px; font-size: 13px; border: 1px solid var(--border); border-radius: var(--radius-sm); min-width: 90px;"
          @change=${e=>{let t=e.target.value;c(i.key,{verboseLevel:t||null})}}
        >
      ...
      <td>
        <select
          ?disabled=${b}
          style="padding: 6px 10px; font-size: 13px; border: 1px solid var(--border); border-radius: var(--radius-sm); min-width: 90px;"
          @change=${e=>{let t=e.target.value;c(i.key,{reasoningLevel:t||null})}}
        >
