...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://metis.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://metis.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://metis.ai/abc-123", baseHref)).toBe(
      "blob:https://metis.ai/abc-123",
    );
  });
