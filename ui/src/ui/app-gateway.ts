...
} from "../metis-runtime/gateway/events.js";
import {
...
export function connectGateway(host: GatewayHost, options?: ConnectGatewayOptions) {
  ...
  const client = new GatewayBrowserClient({
    ...
    onHello: (hello) => {
      ...
      void subscribeSessions(host as unknown as MetisApp);
      void loadAssistantIdentity(host as unknown as MetisApp);
      void loadAgents(host as unknown as MetisApp);
      void loadHealthState(host as unknown as MetisApp);
      if (host.tab === "nodes" || host.tab === "overview") {
        void loadNodes(host as unknown as MetisApp, { quiet: true });
      }
      void loadDevices(host as unknown as MetisApp, { quiet: true });
...
function handleTerminalChatEvent(
  ...
  if (hadToolEvents && state === "final") {
    void loadChatHistory(host as unknown as MetisApp);
    return true;
  }
  return false;
}
...
function handleChatGatewayEvent(host: GatewayHost, payload: ChatEventPayload | undefined) {
  ...
  const state = handleChatEvent(host as unknown as MetisApp, payload);
