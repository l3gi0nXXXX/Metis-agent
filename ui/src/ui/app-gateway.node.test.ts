...
import { GATEWAY_EVENT_UPDATE_AVAILABLE } from "../metis-runtime/gateway/events.js";
import { ConnectErrorDetailCodes } from "../metis-runtime/gateway/protocol/connect-error-details.js";
  ...
  it("returns serverVersion for same-origin websocket targets", () => {
    expect(
      resolveControlUiClientVersion({
        ...
        pageUrl: "http://localhost:8787/metis/",
      }),
    ).toBe("2026.3.7");
  });
  ...
  it("returns serverVersion for same-origin relative targets", () => {
    expect(
      resolveControlUiClientVersion({
        gatewayUrl: "/ws",
        serverVersion: "2026.3.7",
        pageUrl: "https://control.example.com/metis/",
      }),
    ).toBe("2026.3.7");
  });
