...
export async function handleWhatsAppStart(host: MetisApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MetisApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MetisApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MetisApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MetisApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MetisApp): string {
...
function resolveGatewayHttpAuthHeader(host: MetisApp): string | null {
...
function buildGatewayHttpHeaders(host: MetisApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MetisApp) {
...
export function handleNostrProfileToggleAdvanced(host: MetisApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MetisApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MetisApp) {
  const state = host.nostrProfileFormState;
