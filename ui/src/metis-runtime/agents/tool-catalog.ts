...
function buildCoreToolGroupMap() {
  ...
  const metisTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInMetisGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:metis": metisTools,
