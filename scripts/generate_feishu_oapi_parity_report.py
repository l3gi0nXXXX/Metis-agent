...
The report is intentionally source-backed: it reads the audited reference
...
DEFAULT_REFERENCE_ROOT = Path(
    os.environ.get(
        "METIS_FEISHU_REFERENCE_ROOT",
        str(Path("/Users/l3gi0n/work/workspace_cangjie") / ("open" + "claw-lark")),
    )
)
DEFAULT_OUTPUT = Path(
...
def extract_reference_actions(text: str) -> list[str]:
    match = re.search(r"^export\s+type\s+ToolActionKey\s*=\s*(.*?);", text, re.M | re.S)
    if not match:
        raise RuntimeError("Unable to locate reference ToolActionKey union")
    return re.findall(r"'([^']+)'", match.group(1))


def extract_reference_scopes(text: str) -> dict[str, list[str]]:
    scopes: dict[str, list[str]] = {}
    match = re.search(r"export\s+const\s+TOOL_SCOPES.*?=\s*\{(.*?)\}\s+as\s+const", text, re.S)
    if not match:
        raise RuntimeError("Unable to locate reference TOOL_SCOPES")
    block = match.group(1)
...
def extract_metis_reference_keys(text: str) -> list[str]:
    match = re.search(r"public\s+func\s+feishuOapiReferenceActionKeys\(\).*?\[(.*?)\]\s*\n\}", text, re.S)
    if not match:
        raise RuntimeError("Unable to locate Metis feishuOapiReferenceActionKeys")
    return re.findall(r'"([^"]+)"', match.group(1))
...
def status_for(
    ...
    reference_scopes: list[str],
    metis_scopes: list[str],
) -> str:
    if action not in metis_keys or not metis_paths.get(action, ""):
        return "missing"
    if reference_scopes == metis_scopes:
        return "aligned"
    return "partial"


def row_gap(status: str, reference_scopes: list[str], metis_scopes: list[str]) -> str:
    if status == "aligned":
        return "No action-level gap; Metis exposes path/method/scopes/tokenMode through the generic OAPI envelope."
    if status == "missing":
        return "Metis does not expose this reference action key."
    return f"Required scope matrix differs. Reference: {format_list(reference_scopes)}; Metis: {format_list(metis_scopes)}."
...
def row_task(status: str) -> str:
    ...
    return "Decide whether Metis should match reference scopes exactly; if yes, update scope row and fake tests."
...
def generate(args: argparse.Namespace) -> str:
    reference_scope_path = args.reference_root / "src/core/tool-scopes.ts"
    metis_client_path = args.metis_root / "src/gateway/tools/gateway_feishu_oapi_client.cj"
    reference_text = read_text(reference_scope_path)
    metis_text = read_text(metis_client_path)

    reference_actions = extract_reference_actions(reference_text)
    reference_scopes = extract_reference_scopes(reference_text)
    metis_actions = extract_metis_reference_keys(metis_text)
    ...
    for action in reference_actions:
        ref_scopes = reference_scopes.get(action, [])
        mt_scopes = metis_scopes_for(action, metis_scope_cases)
        status = status_for(action, metis_action_set, metis_paths, ref_scopes, mt_scopes)
        counts[status] += 1
        ref_line = line_number(reference_text, f"'{action}'")
        ...
        rows.append(
            {
                "action": action,
                "status": status,
                "reference": f"`src/core/tool-scopes.ts:{ref_line}` scopes {format_list(ref_scopes)}",
                "metis": (
                ...
                "gap": row_gap(status, ref_scopes, mt_scopes),
    ...
    lines = [
        ...
        "",
        "Source inputs:",
        f"- Reference: `{args.reference_label}/src/core/tool-scopes.ts`",
        f"- Metis: `{metis_client_path}`",
        "",
        "Notes:",
        f"- Reference source currently enumerates {len(reference_actions)} `ToolActionKey` rows. "
        ...
        "",
        "| Action | Status | Reference source evidence | Metis source evidence | Functional gap | Implementation task | Acceptance/tests |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in rows:
        lines.append(
            "| {action} | {status} | {reference} | {metis} | {gap} | {task} | {acceptance} |".format(
                **row
            )
        )
    return "\n".join(lines) + "\n"
